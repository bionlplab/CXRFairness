        ...
        for phase in ['train', 'val']:
            ...
            if phase == "val":
                ...
                if best_AUROC_avg < AUROC_avg:
                    ...
                    torch.save(model.state_dict(), "/prj0129/mil4012/MIDRC/weights/densenet121_sex_balance_midrc_crossnew1.pth")
...
def test_model(test_loader,model):
    ...
    gt1 = gt.cpu()
    gt2 = gt1.numpy()
    np.savetxt('/prj0129/mil4012/MIDRC/Result/densenet121_sex_balance_midrc_crossnew1.txt', pred2)
    np.savez('/prj0129/mil4012/MIDRC/Result/densenet121_sex_balance_midrc_crossnew1.npz', prediction=pred2, label=gt2, group=groups)
...
if __name__ == '__main__':
    ...
    ## add data
    index = np.argwhere(train_groups==1)
    index = np.reshape(index,(len(index),))
#     diff = len(train_groups) - len(index)
    diff = len(np.argwhere(train_groups==0)) - len(index)
    print('the len of training group is',len(train_groups))
    print('the len of training group 1 is',len(index))
    
    a = index[np.random.choice(len(index),diff)]
    train_path = np.concatenate((train_path, train_path[a]), axis=0)
    train_labels = np.concatenate((train_labels, train_labels[a]), axis=0)
    train_groups = np.concatenate((train_groups, train_groups[a]), axis=0)
...
#     model_ft = train_model(dataloaders, model_ft, criterion, optimizer_ft,num_epochs=20)
    model_ft.load_state_dict(torch.load("/prj0129/mil4012/MIDRC/weights/densenet121_sex_balance_midrc_crossnew1.pth"))
